...
ACTIONS = ("click", "scroll", "drag", "type", "press", "wait", "finish", "ask_human")
...
# 下面这段提示词里写了"单次等待上限 30 秒""一个回合 50 步"——
# 这两个数字来自 run.py 的 `_wait`（min(30.0, ...)）和 `MAX_STEPS`。
...
- {"type": "scroll", "notches": 3}                       滚轮，正数向下，负数向上（一"格"约 100px）
...
- {"type": "wait", "seconds": 5}                         等待，给页面反应时间（单次上限 30 秒）
- {"type": "finish", "reason": "说明"}                   目标已完成
- {"type": "ask_human", "question": "要人做什么"}         需要人来处理（登录、验证码等）

看视频 / 长等待（网课、剧集这类"要等它播完"的任务）：
- 先看清播放器上的**总时长**、**当前进度**和**倍速**（例如 12:34 / 45:00、1.5x）。
  还要等多久 ≈（总时长 − 当前进度）÷ 倍速。
- 播放正常时**每次都把等待拉满**（单次上限 30 秒），别用 3、5 秒的小碎步——那是白耗步数。
  只有"页面在加载"这种几秒就好转的情况才用短等待。
- **每次等完，先核对播放器上的时间有没有往前走**（进度条末端的数字变了没）：
  - 往前走了 → 正常，接着等下一次。
  - 没往前走 → 视频卡住了或播放出错（弹窗、需要点"继续"），**立刻停止等待去处理**，
    不要继续干等下去。
- 一个回合总共 50 步。视频比这还长的话本来就等不完：把当前进度写进 finish，
  让人再发一句接着跑，别硬等。
