...
- **怎么读坐标**：网格有两层——每 100px 一条**深色线并标着数字**（100、200、300…），
  中间每 20px 有一条**淡色细线**。
  要读某个点的坐标：先看它左右最近的那条**数字线**，再数中间隔了几条细线，
  每条细线是 20px。例如目标在 200 这条线右边第 2 条细线 -> x = 240。
  **不要估**，数格子——这是精确读数，估出来的值经常差几十像素点不中。
- 网格上的数字就是那个位置的坐标（页面像素，原点在视口左上角）。
  要点击某个点，照上面那样读出 x、y 填上即可——**不用做任何加减**。
- **小元素（标签页、按钮、单选圈、勾选框）要量左右边界再取中点**，别猜中心：
  一个「章节测验」标签可能只有 56px 宽（比如横跨 186~242，中心就是 214），
  凭感觉写 280 就点到隔壁去了（实测踩过）。做法是看它左边缘压在哪条线上、
  右边缘压在哪条线上，两个数取平均。
- 点偏了往往没反应，看着像"页面没变化"，于是白白重复好几次。
  所以**第一下就要读准**；如果点了两三次都没动静，先怀疑坐标读偏了，
  换个位置重读，别一直重复同一个数。
- **标签页/按钮这类小元素特别容易点偏**。它们的宽度往往只有几十像素，
  偏 20px 就点到隔壁去了（实测踩过：点「章节测验」标签，读成 280，
  实际中心在 214，整整偏了 66px，结果点到旁边的「1 视频」上，
  一直以为页面没反应）。读这类小元素时**先量它的左右边界**，
  取中间值，别直接猜中心。
